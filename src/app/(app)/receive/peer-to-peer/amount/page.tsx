...
export default function PeerToPeerAmountPage() {
  ...
  const peerPhone = searchParams.get("phone") || "";
  ...
  const handleRequest = () => {
    ...
    console.log("Request amount:", amount, "from:", peerName);
  };
    ...
    <div className="flex flex-col w-full flex-1 bg-black min-h-full py-6">
      ...
      <div className="flex flex-col gap-6 px-4 overflow-y-auto pb-6">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-white text-sm font-medium">Requested Account</label>
          <div className="bg-linear-to-b from-[#161616] to-[#0F0F0F] border border-white/20 rounded-2xl p-4 flex items-center gap-4">
            ...
            <div className="flex flex-col flex-grow">
              <span className="text-white font-medium">{peerName}</span>
              <span className="text-gray-400 text-sm">{peerPhone}</span>
            </div>
          </div>
        </div>
