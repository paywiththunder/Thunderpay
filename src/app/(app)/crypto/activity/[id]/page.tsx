...
export default function ActivityDetailPage({ params }: { params: Promise<{ id: string }> }) {
            ...
            try {
                ...
                responses.forEach((res) => {
                    if (res && res.success && res.data && Array.isArray(res.data.items)) {
                        const tx = res.data.items.find((t: any) => t.reference === id);
                        if (tx) foundTransaction = tx;
                    }
                });
    ...
    const isNegative = transaction && ["send", "withdrawal", "bill", "electricity", "card"].includes(transaction.source.toLowerCase());
                    ...
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <div className="text-center">
                            ...
                            <p className={`text-3xl font-bold ${isNegative ? "text-red-500" : "text-green-500"}`}>
                                {isNegative ? "-" : "+"}{transaction.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 8 })}
                            </p>
                        </div>
