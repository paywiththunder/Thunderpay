...
export default function TVPage() {
    ...
    useEffect(() => {
            ...
            if (decoderNumber.length >= 10 && selectedProvider) {
                try {
                    const response = await verifyTv({
                        serviceId: selectedProvider.id.toUpperCase(),
        ...
        const timer = setTimeout(verifyDecoder, 1000);
    ...
    const getFilteredPlans = () => {
        ...
        return availablePlans.filter((plan) => {
            ...
            switch (paymentOption) {
                case "monthly": return durationLower === "1 month" || durationLower.includes("month") && !durationLower.includes("3") && !durationLower.includes("year");
                case "quarterly": return durationLower.includes("quarter") || durationLower.includes("3 month");
                case "hot-offers":
                default: return durationLower.includes("month") && !durationLower.includes("3");
            }
        });
    };
    ...
    const handlePaymentMethodSelect = (method: PaymentOption) => {
        ...
        if (selectedPlan && decoderNumber) {
            ...
            const payload: TvQuotePayload = {
                ...
                subscription_type: ["DSTV", "GOTV"].includes(serviceId) ? "change" : null
            };
    ...
    const calculatePaymentAmount = (): string => {
        ...
        if (selectedPaymentMethod.type === "fiat") return `₦${selectedPlan.price.toLocaleString()}.00`;
        return `₦${selectedPlan.price.toLocaleString()}.00`;
    };
    ...
    const handlePinComplete = (pin: string) => {
        ...
        if (storedQuote) {
            ...
            paymentMutation.mutate({ quoteReference: parsed.quoteReference, pin });
        }
    };
    ...
    if (step === "payment") return <PaymentMethod onBack={() => setStep("form")} onSelect={handlePaymentMethodSelect} amount={selectedPlan?.price || 0} walletType="fiat" />;
    ...
    if (step === "confirmation" && selectedPaymentMethod && selectedPlan) {
        ...
        if (quoteData?.deductionAmount && selectedPaymentMethod.type !== "fiat") displayAmount = `${Number(quoteData.deductionAmount).toFixed(6)} ${quoteData.deductionCurrency}`;
        return <Confirmation onBack={() => setStep("payment")} onPay={() => setStep("enterPin")} amount={selectedPlan.price} paymentAmount={displayAmount} availableBalance={selectedPaymentMethod.balance || selectedPaymentMethod.value} boltBalance={boltBalance} details={details} />;
    }

    if (step === "enterPin") return <EnterPin onBack={() => setStep("confirmation")} onComplete={handlePinComplete} isLoading={isProcessing} error={pinError} />;

    if (step === "result" && selectedPaymentMethod && selectedPlan) {
        let paymentAmount = calculatePaymentAmount();
        if (quoteData?.deductionAmount && selectedPaymentMethod.type !== "fiat") paymentAmount = `${Number(quoteData.deductionAmount).toFixed(6)} ${quoteData.deductionCurrency}`;
        const commonDetails = [
            { label: "Biller", value: selectedProvider.name },
            { label: "Smartcard Number", value: decoderNumber },
            { label: "Account Name", value: customerName || "N/A" },
    ...
    return (
        <div className="flex flex-col w-full flex-1 bg-black min-h-full py-6">
            ...
            <div className="flex flex-col gap-6 px-4 overflow-y-auto pb-6">
                    ...
                    <div className="relative" ref={dropdownRef}>
                        <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="w-full bg-linear-to-b from-[#161616] to-[#0F0F0F] border border-white/20 shadow-[inset_0_1px_4px_rgba(255,255,255,0.1)] rounded-2xl p-4 flex items-center justify-between cursor-pointer hover:bg-gray-800/50 transition-colors">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center flex-shrink-0"><span className="text-white text-xs font-bold">{selectedProvider.logo}</span></div>
                            ...
                            <div className="absolute top-full left-0 right-0 mt-2 bg-linear-to-b from-[#161616] to-[#0F0F0F] border border-white/20 rounded-2xl shadow-lg z-10 max-h-60 overflow-y-auto">
                                {cableTVProviders.map((provider) => (
                                    <button key={provider.id} onClick={() => { setSelectedProvider(provider); setIsDropdownOpen(false); }} className="w-full p-4 flex items-center gap-3 hover:bg-gray-800/50 transition-colors first:rounded-t-2xl last:rounded-b-2xl">
                                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center flex-shrink-0"><span className="text-white text-xs font-bold">{provider.logo}</span></div>
                ...
                <div className="flex flex-col gap-2">
                    <label className="text-white text-sm font-medium">Decoder Number</label>
                    <input type="text" value={decoderNumber} onChange={(e) => setDecoderNumber(e.target.value)} placeholder="e.g 00000000000" className="w-full bg-linear-to-b from-[#161616] to-[#0F0F0F] border border-white/20 text-white placeholder-gray-500 px-4 py-3.5 rounded-2xl focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all text-sm" />
                    {decoderVerified && customerName && <div className="flex items-center gap-2"><HiCheckCircle className="w-4 h-4 text-blue-500" /><span className="text-white font-medium text-sm">{customerName}</span></div>}
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <input type="tel" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} placeholder="e.g 08012345678" className="w-full bg-linear-to-b from-[#161616] to-[#0F0F0F] border border-white/20 text-white placeholder-gray-500 px-4 py-3.5 rounded-2xl focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all text-sm" />
                </div>
                <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-0 bg-linear-to-b from-[#161616] to-[#0F0F0F] border border-white/20 rounded-2xl overflow-hidden">
                        <button onClick={() => setPaymentOption("hot-offers")} className={`flex-1 py-3 px-4 text-sm font-medium transition-colors ${paymentOption === "hot-offers" ? "bg-gray-700 text-white" : "text-white/70 hover:text-white"}`}>Hot Offers</button>
                        <div className="w-px bg-white/20"></div>
                        <button onClick={() => setPaymentOption("monthly")} className={`flex-1 py-3 px-4 text-sm font-medium transition-colors ${paymentOption === "monthly" ? "bg-gray-700 text-white" : "text-white/70 hover:text-white"}`}>Monthly</button>
                        <div className="w-px bg-white/20"></div>
                        <button onClick={() => setPaymentOption("quarterly")} className={`flex-1 py-3 px-4 text-sm font-medium transition-colors ${paymentOption === "quarterly" ? "bg-gray-700 text-white" : "text-white/70 hover:text-white"}`}>Quarterly</button>
                    </div>
                </div>
                <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex items-center gap-3 bg-linear-to-b from-[#161616] to-[#0F0F0F] border border-white/20 rounded-2xl p-2 px-3">
                            ...
                            <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-8 h-8 flex items-center justify-center text-white bg-gray-800/50 hover:bg-gray-700/80 rounded-full transition-colors border border-white/10">-</button>
                            <span className="text-white text-sm font-bold min-w-[70px] text-center">{quantity} {quantity > 1 ? "Months" : "Month"}</span>
                            <button onClick={() => setQuantity(quantity + 1)} className="w-8 h-8 flex items-center justify-center text-white bg-gray-800/50 hover:bg-gray-700/80 rounded-full transition-colors border border-white/10">+</button>
                        </div>
                    </div>
                    <div className="grid grid-cols-3 gap-3">
                        {isPlansLoading ? <div className="col-span-3 flex justify-center py-8"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-white"></div></div> : filteredPlans.length > 0 ? filteredPlans.map((plan, i) => (
                            <button key={`${plan.id}-${i}`} onClick={() => setSelectedPlan(plan)} className={`bg-linear-to-b from-[#161616] to-[#0F0F0F] border border-white/20 rounded-2xl p-4 flex flex-col items-center justify-center hover:bg-gray-800/50 transition-colors ${selectedPlan?.id === plan.id ? "ring-2 ring-blue-500 border-blue-500" : ""}`}>
                ...
                <button onClick={(e) => { e.preventDefault(); if (decoderNumber && selectedPlan && decoderVerified && phoneNumber) setStep("payment"); }} disabled={!decoderNumber || !selectedPlan || !decoderVerified || !phoneNumber} className={`w-full py-4 rounded-full font-bold text-white transition-all mt-4 mb-20 bg-linear-to-b from-[#161616] to-[#0F0F0F] border border-white/20 shadow-[inset_0_1px_4px_rgba(255,255,255,0.1)] hover:bg-gray-800/50 ${!decoderNumber || !selectedPlan || !decoderVerified || !phoneNumber ? "bg-gray-900 text-gray-600 border border-gray-800 cursor-not-allowed" : ""}`}>Pay</button>
            </div>
        </div>
    );
}
